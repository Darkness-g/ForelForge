...
def home():
    if session.get('username'):
        ...
        try:
            cursor.execute("""
                SELECT game_id, title, description, price,
                       release_date, developer, image_url
                FROM Games
    ...
    if request.method == "POST":
        ...
        if not title or not description or not game_file:
            ...
            game_filename = secure_filename(
                f"{datetime.now().timestamp()}_{game_file.filename}"
            )
            ...
            try:
                cursor.execute("""
                    ...
                    (title, description, price, release_date, developer,
                     image_url, system_requirements, file_path, status)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    title,
                    description,
...
def pending_game(game_id):
    ...
    try:
        cursor.execute("""
            SELECT game_id, title, description, price, image_url, file_path
            FROM Games
        ...
        selected_game = next(
            (g for g in pending_games if g["game_id"] == game_id),
            None
        )
...
def approve_game(game_id):
    ...
    try:
        ...
        approved_game_path = os.path.join(
            APPROVED_FOLDER,
        ...
        if image_url != "images/default_game.png":
            ...
            if os.path.exists(old_image_path):
                ...
                approved_image_path = os.path.join(
                    APPROVED_FOLDER,
                ...
                approved_image_db = (
                    f"uploads/approved/{approved_image_filename}"
                )
            else:
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO Users
                (username, email, password_hash, avatar_url, balance)
                VALUES (?, ?, ?, ?, ?)
            """, (
                username,
                email,
...
def new_topic():
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute(
                "INSERT INTO Topics (title, creator_id) VALUES (?, ?)",
                (title, creator_id)
            )
            conn.commit()
...
def download_game(game_id):
    ...
    try:
        if session.get('role') == 'admin':
            cursor.execute(
                "SELECT file_path, title FROM Games WHERE game_id=?",
                (game_id,)
            )
        else:
            cursor.execute("""
...
def profile():
    ...
    try:
        cursor.execute(
            "SELECT username, avatar_url, balance FROM Users WHERE id=?",
            (session['id'],)
        )
    ...
    for game in purchased_games_list:
        user_games.append({
            ...
            'image': url_for(
                'static',
                filename=game.image_url.replace('static/', '')
            ) if game.image_url else url_for(
                'static',
                filename='uploads/images/default_game.png'
            )
        })
